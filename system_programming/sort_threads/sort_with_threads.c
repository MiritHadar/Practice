...
int main(int argc, char *argv[])
{
    ...
    /*copying the origin file once in to a buffer */
    fseek(fp, 0, SEEK_END);
    buffer_size = ftell(fp) - 1;

    buf = malloc(buffer_size * sizeof(char*) * NUM_OF_DUP);
    if (NULL == buf)
    {
    ...
    /* creating NUM_OF_DUP copies of the origin file */
    for (i = 0; i < NUM_OF_DUP; ++i)
    {
        fseek(fp, 0, SEEK_SET);
        fread(buf + i * buffer_size, sizeof(char*), buffer_size, fp);
    }

    buf[buffer_size * NUM_OF_DUP] = '\0';
    fseek(fp, 0, SEEK_SET);
 ...
 */    
    ...
    last_start = ptrs_to_file;
    /* test for one thread 
    arr_args[0].start = ptrs_to_file;
    arr_args[0].size = num_of_lines;

    SortThread(&arr_args[0]);*/
     
    for (i = 0; i < NUM_OF_THREADS - 1; ++i)
    {
        arr_args[i].start = last_start + last_size;
        arr_args[i].size = num_of_lines / NUM_OF_THREADS;
        last_size = arr_args[i].size;
        last_start = arr_args[i].start;
    }

    /* last thread */
    arr_args[i].start = last_start + last_size;
    arr_args[i].size = arr_args[i].start - last_start;
