...
public:
    ...
    PublicConvoy(const PublicConvoy& other_)
        : m_pt1(new Minibus(*reinterpret_cast<Minibus*>(other_.m_pt1)))
        , m_pt2(new Taxi(*reinterpret_cast<Taxi*>(other_.m_pt2)))
        , m_m(other_.m_m), m_t(other_.m_t)
    {
    }

/* the jenerated CCtor isnt good. needs deep copy and not shallow copy. need to locate room on the CCtor */
  ~PublicConvoy()
    {
        delete m_pt2;
        delete m_pt1;
